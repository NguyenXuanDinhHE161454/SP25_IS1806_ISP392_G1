                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        <c:forEach var="debt" items="${debtList}">
                                            <tr class="${debt.totalDebt < 0  ? 'table-success' : 'table-danger'}">
                                                <td>${debt.customerId}</td>
                                                <td>${debt.customerName}</td>
                                                <td>${debt.phoneNumber}</td>
                                                <td>${debt.totalDebt < 0 ? '-' : '+'}</td>
                                                <td>
                                                    <fmt:formatNumber value="${debt.totalDebt}" type="currency" currencySymbol="VNĐ" />
                                                </td>
                                                <td>
                                                    <a href="DebtController?action=getDebtByCustomer&customerId=${debt.customerId}" class="btn btn-primary btn-sm">Details</a>
                                                </td>
                                            </tr>
                                        </c:forEach>
                                    </tbody>
